...
var serverKey = 'AAAAjzPid4U:APA91bHdKb4V2c6bBj4njU5dm1lcgsSptIGQvbtXjZVw64TpGPpMdvvOw-UdLv0BNE9hnVEEA9fF0qcC_c90STADQyxsv-dREzhTBUqcxMN_Bxa9-XUGZqp_pKoh37CRz42XkG-CQwIS'; //put your server key here
...
Models.EmployeeVisitBeforeAfterPhotos.hasOne(Models.EmployeeVisitBeforeAfterPhotoComments,{
  	foreignKey: 'photo_id',
									...
									if(authAttempt){
										if(empData.status != null && empData.status == 'active'){
											if(empData.app_device_id != null && empData.app_device_id != ''){
												...
												empData.update({app_device_id:app_device_id, online_status:"online"}).then(updatedEmp => {
													res.end(JSON.stringify({
												                            response: 1,
												                            message: Messages['en'].LOGIN_SUCCESS,
												                            result : empData
		...
		if(current_visit_status == 'started'){
			Models.EmployeeVisits.findOne({
				...
				if(employeeCheck !=  null){
					...
					if(req.body.battery_life != "" && req.body.battery_life != null && req.body.android_version != "" && req.body.android_version != null && req.body.latitude != "" && req.body.latitude != null && req.body.longitude != "" && req.body.longitude != null && req.body.address != "" && req.body.address != null){
						Models.EmployeeVisits.create({
							...
							visit_type:req.body.visit_type ? req.body.visit_type : "normal",
							...
							console.log("employee visit create error ============== ",error)
							res.end(JSON.stringify({
				                        response: 2,
				                        message: Messages['en'].WRONG_DATA
				                    }));
...
exports.add_photo_during_visit = function(req, res){
	if(req.body.user_id != "" && req.body.user_id != null && req.body.employee_id != "" && req.body.employee_id != null && req.body.customer_id != "" && req.body.customer_id != null && req.files && req.body.visit_id != "" && req.body.visit_id != null){
        ...
        Models.EmployeeVisitPhotos.create(req.body).then(employeeVisitPhoto => {
        	console.log("employee visit photo id =================",employeeVisitPhoto);
	...
	if(req.body.user_id != null && req.body.user_id != "" && req.body.item_id != null && req.body.item_id != "" && req.body.employee_id != null && req.body.employee_id != "" && req.body.customer_id != null && req.body.customer_id != "" && req.body.unit_id != null && req.body.unit_id != "" && req.body.quantity != null && req.body.quantity != "" && req.body.total_price != null && req.body.total_price != "" && req.body.total_price_before_tax != null && req.body.total_price_before_tax != "" && req.body.total_tax != null && req.body.total_tax != "" && req.body.tax_type != null && req.body.tax_type != "" && req.body.base_price_per_unit != null && req.body.base_price_per_unit != ""){
		...
		Models.SalesOrderCartDetail.findOne({
			...
			if(checkItem == null){
				...
				create_sales.discount_percent = req.body.discount_percent ? req.body.discount_percent : null;
				...
				let update_sales = {};
				console.log('update else >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>',req.body.bonus_qty,req.body.discount);
				...
				update_sales.discount_percent = req.body.discount_percent ? req.body.discount_percent : null;
...
exports.adjust_items_from_sales_cart = function(req, res){
	...
	if(req.body.user_id != null && req.body.user_id != "" && req.body.item_id != null &&
		req.body.item_id != "" && req.body.employee_id != null && req.body.employee_id != "" &&
		req.body.customer_id != null && req.body.customer_id != "" && req.body.unit_id != null &&
	   	req.body.unit_id != "" && req.body.quantity != null && req.body.quantity != "" &&
		req.body.total_price != null && req.body.total_price != "" && 
		req.body.total_price_before_tax != null && req.body.total_price_before_tax != "" && 
		req.body.total_tax != null && req.body.total_tax != "" && req.body.tax_type != null &&
		req.body.tax_type != "" && req.body.base_price_per_unit != null &&
			...
			if(checkItem == null){
				...
				*/
				...
				if(checkItem.unit_id != req.body.unit_id) {
					res.end(JSON.stringify({ response: 2, message: 'Please select proper unit id of this invoice' }) );
				}
				// quantity
				else if(checkItem.quantity < req.body.quantity){
					res.end(JSON.stringify({ response: 2, message: 'Quantity should be less than' + checkItem.quantity }) );
				}
				// bonus quantity
				else if(req.body.bonus_qty != null && req.body.bonus_qty != "" && checkItem.bonus != null && checkItem.bonus < req.body.bonus_qty){
					res.end(JSON.stringify({ response: 2, message: 'Bonus Quantity should be less than' + checkItem.bonus }) );
				}
				else if(req.body.bonus_qty != null && req.body.bonus_qty != "" && checkItem.bonus == null && req.body.bonus_qty > 0){
					res.end(JSON.stringify({ response: 2, message: 'Quantity should be 0' }) );
				}
				// discount
				else if(req.body.discount != null && req.body.discount != "" && checkItem.discount != null && checkItem.discount < req.body.discount){
					res.end(JSON.stringify({ response: 2, message: 'Discount should be less than' + checkItem.discount }) );
				}
				else if(req.body.discount != null && req.body.discount != "" && checkItem.discount == null && req.body.discount > 0){
					res.end(JSON.stringify({ response: 2, message: 'Discount should be 0' }) );
				}
				// discount percent
				else if(req.body.discount_percent != null && req.body.discount_percent != "" && checkItem.discount_percent != null && checkItem.discount_percent < req.body.discount_percent){
					res.end(JSON.stringify({ response: 2, message: 'Discount Percent should be less than' + checkItem.discount_percent }) );
				}
				else if(req.body.discount_percent != null && req.body.discount_percent != "" && checkItem.discount_percent == null && req.body.discount_percent > 0){
					res.end(JSON.stringify({ response: 2, message: 'Discount Percent should be 0' }) );
				}
				// base price per unit should be exactly same
				else if(req.body.base_price_per_unit != checkItem.base_price_per_unit){
					res.end(JSON.stringify({ response: 2, message: 'Base price per unit should be equal to' + checkItem.base_price_per_unit }) );
				}
				// tax type should be same
				else if(req.body.tax_type != checkItem.tax_type){
					res.end(JSON.stringify({ response: 2, message: 'Tax type should be same' }) );
				} else{
					...
					const c_discount_percent = req.body.discount_percent ? req.body.discount_percent : null;

					if(c_bonus != null && checkItem.bonus != null) update_sales.bonus = checkItem.bonus - c_bonus;
					else update_sales.bonus =c_bonus

					if(c_discount != null && checkItem.discount != null) update_sales.discount = checkItem.discount - c_discount
					else update_sales.discount = c_discount

					if(c_discount_percent != null && checkItem.discount_percent != null) update_sales.discount_percent = checkItem.discount_percent - c_discount_percent  
					...
					*/
					update_sales.total_price = checkItem.total_price - req.body.total_price;
					update_sales.total_price_before_tax = checkItem.total_price_before_tax - req.body.total_price_before_tax;
					...
					*/
					...
					if(req.body.total_price_with_tax !="" && req.body.total_price_with_tax != null){
						update_sales.total_price_with_tax = checkItem.total_price_with_tax - req.body.total_price_with_tax;
					}
...
exports.sales_order_request_submit = function(req, res){
	...
	if(req.body.sales_order_arr && req.body.user_id != "" && req.body.user_id != null && req.body.employee_id != "" && req.body.employee_id != null && req.body.customer_id != "" && req.body.customer_id != null && req.body.supervisor_id != "" && req.body.supervisor_id != null && req.body.store_id != "" && req.body.store_id != null && req.body.level != "" && req.body.level != null){
		...
		create_sales.total_price = req.body.total_price;
		create_sales.request_type = req.body.request_type=="invoice" ? "invoice" : req.body.request_type=="return_invoice" ? "return_invoice" : "sales";
		...
		Models.SalesOrderRequest.create(create_sales).then(async salesOrderRequest => {
			if(salesOrderRequest.id){
				...
				var timeline_content_type = req.body.request_type=="invoice" ? "invoice_order" : req.body.request_type=="return_invoice" ? "return_invoice_order" : "sales_order";
	   ...
	   //      	})
				...
				for (var i = 0; i < sales_order_arr.length; i++) {
					...
					if(sales_order_arr[i].item_id && sales_order_arr[i].category_id && sales_order_arr[i].measurement_unit_id && sales_order_arr[i].quantity){
						...
						if(sales_order_arr[i].bonus != null && sales_order_arr[i].bonus != ""){
						...
						if (req.body.request_type=="invoice" && sales_order_arr[i].quantity!=0) {
							Models.StockItems.findOne({
								...
								if (stockItemData.quantity>=sales_order_arr[i].quantity) {
									console.log("if sales order quantity ===================");
									...
									Models.StockItems.update({
										...
										}).then(createdSalesOrderInvoiceRequestStockItems => {
											console.log("error in sales order invoice request stock items create ==================");
										},error => {
											console.log("error in sales order invoice request stock items create ==================");
										})
					...
					if(sales_order_arr.length - 1 == i ){
						...
						Models.Timelines.create({
			        		...
			        		let notify_title = req.body.request_type=="invoice" ? "Invoice Request" : req.body.request_type=="return_invoice" ? "Return Invoice Request" : "Sales Order Request";
			        		let notify_desc = req.body.request_type=="invoice" ? "You Have New Invoice Request" : req.body.request_type=="return_invoice" ? "You Have New Return Invoice Request" : "You Have New Sales Order Request";
							...
							// try{
								if(salesOrderRequest.supervisor_id != null && salesOrderRequest.supervisor_id != ""){
			...
			if(salesRequestChk == null){
				...
				if(req.body.status == "accepted" || req.body.status == "rejected"){
					salesRequestChk.update({
						...
						let gcm_obj = {};
						gcm_obj.req_type = 'supervisor_sales_order_action',gcm_obj.action = 'supervisor';
						if(salesRequestChk.employee_id != null && salesRequestChk.employee_id != ""){
							...
							Models.GcmDevices.findOne({
								...
								if(gcmDev != null && gcmDev.device_token != null){
									_sendPushNotificationAndroid(gcmDev.device_token, 'Sales Order Request', 'Sales Order request no. '+salesRequestChk.id+' has been '+req.body.status, gcm_obj);
								}else{
									console.log('data is null in gcmDev supervisor');
								}
							},error => {
						...
						if(salesRequestChk.salesmanager_id != null && salesRequestChk.salesmanager_id != ""){
							console.log('supervisor_id',salesRequestChk.salesmanager_id)
							Models.GcmDevices.findOne({
								where:{
			...
			if(salesRequestChk == null){
				...
				if(req.body.status == "accepted" || req.body.status == "rejected"){
					salesRequestChk.update({
						...
						let gcm_obj = {};
						gcm_obj.req_type = 'salesmanager_sales_order_action',gcm_obj.action = 'salesmanager';
						if(salesRequestChk.employee_id != null && salesRequestChk.employee_id != ""){
							console.log('supervisor_id',salesRequestChk.employee_id)
							Models.GcmDevices.findOne({
								where:{
						...
						if(salesRequestChk.supervisor_id != null && salesRequestChk.supervisor_id != ""){
...
exports.submit_payment = function(req, res){
	...
	if (req.body.user_id!=null && req.body.user_id!="" && req.body.employee_id!=null && req.body.employee_id!="" && req.body.customer_id!=null && req.body.customer_id!="" && req.body.payment_type!=null && req.body.payment_type!="") {
		...
		if (req.body.payment_type=="cheque") {
			...
			console.log("req body under cheque =================",req.body);
				...
				console.log("success ===================");
	   			...
	   			Models.Timelines.create({
		            ...
		            Models.Employees.findOne({
		            	...
		            	console.log("under employee data =====================")
		            	...
		            	Models.Customers.findOne({
		            		...
		            		console.log("under customer data =====================")
		            		let credit_amount;
		            		console.log("req amount ===============",req.body.amount)
		            		console.log("credit limit ===============",customerData.credit_limit)
		            		if (req.body.amount!=null && req.body.amount!="") {
		            			credit_amount = parseFloat(customerData.credit_limit!=null ? customerData.credit_limit: 0 )+parseFloat(req.body.amount);	            			
		            		}else{
		            		...
		            		Models.Customers.update({
		            			...
		            			console.log("under update limit =====================")
		            			let gcm_obj = {};
								gcm_obj.req_type = 'payment',gcm_obj.action = 'submit';
								
								if(submitPaymentData.supervisor_id != null && submitPaymentData.supervisor_id != ""){
									console.log('supervisor_id ===================',submitPaymentData.supervisor_id)
									Models.GcmDevices.findOne({
										where:{
										...
										if(gcmDev != null && gcmDev.device_token != null){
											console.log("push notification function ======================")
											_sendPushNotificationAndroid(gcmDev.device_token, 'Payment', 'Payment is done via '+submitPaymentData.payment_type+' of '+submitPaymentData.amount+' by '+employeeData.employee_name_en, gcm_obj);
										}else{
											console.log('data is null in gcmDev supervisor');
										}
									},error => {
			...
			req.body.reference_no = "RF_"+Date.now() + '_' + Math.floor(10000 + Math.random() * 90000);
			req.body.payment_no = "PN_"+Date.now() + '_' + Math.floor(10000 + Math.random() * 90000);
			console.log("req body under cash =================",req.body);
			Models.CustomerPayments.create({
				user_id:req.body.user_id,
				employee_id:req.body.employee_id,
				customer_id:req.body.customer_id,
				...
				console.log("success cash payment ===================");
	        		...
	        		longitude:req.body.longitude,
		         	...
		         	Models.Employees.findOne({
			            ...
			            Models.Customers.findOne({
		            		...
		            		let credit_amount;
		            		if (req.body.amount!=null && req.body.amount!="") {
		            			credit_amount = parseFloat(customerData.credit_limit)+parseFloat(req.body.amount);	            			
		            		}else{
		            			credit_amount = customerData.credit_limit;
		            		}
		            		Models.Customers.update({
				...
				console.log("error cash payment ===================",error);
				res.end(JSON.stringify({
	                response: 0,
	                message: Messages['en'].ERROR_CREATE
	            }));
...
exports.timeline_for_salesman = function(req, res){
	if (req.body.user_id!=null && req.body.user_id!="" && req.body.employee_id!=null && req.body.employee_id!="") {
...
exports.timeline_for_specific_customer = function(req, res){
	if (req.body.user_id!=null && req.body.user_id!="" && req.body.employee_id!=null && req.body.employee_id!="" && req.body.customer_id!=null && req.body.customer_id!="") {
...
exports.task_seen = function(req, res){
	if (req.body.task_id!=null && req.body.task_id!="" && req.body.seen_status!=null && req.body.seen_status!="" && req.body.seen_date_time!=null && req.body.seen_date_time!="") {
		...
		Models.Tasks.update({
			...
			Models.Tasks.findOne({
				...
				console.log("specific success =====================",updatedTaskData);
				res.end(JSON.stringify({
		            response: 1,
		            message: Messages['en'].SUCCESS_FETCH,
...
exports.task_completed = function(req, res){
	if (req.body.task_id!=null && req.body.task_id!="" && req.body.complete_status!=null && req.body.complete_status!="" && req.body.complete_date_time!=null && req.body.complete_date_time!="") {
		console.log("specific completed under if =========================",req.body);
		Models.Tasks.update({
...
exports.get_surveys = function(req, res){
	if (req.body.user_id!=null && req.body.user_id!="" && req.body.customer_id!=null && req.body.customer_id!="") {
		console.log("specific under if =========================",req.body);
...
exports.get_survey_questions = function(req, res){
	if (req.body.survey_id!=null && req.body.survey_id!="") {
		...
		}).then(surveyQuestionData => {
			console.log("specific success =====================",surveyQuestionData);
			res.end(JSON.stringify({
	            response: 1,
	            message: Messages['en'].SUCCESS_FETCH,
...
exports.submit_survey = function(req, res){
	...
	if (req.body.survey_id!=null && req.body.survey_id!="" && req.body.user_id!=null && req.body.user_id!="" && req.body.employee_id!=null && req.body.employee_id!="" && req.body.customer_id!=null && req.body.customer_id!="" && req.body.answer_data && req.body.answer_data.length>0) {
		...
		for (var i = 0; i < req.body.answer_data.length; i++) {
			...
			Models.SurveyQuestionAnswers.create({
				...
				Models.SurveyAssignToCustomers.update({
					...
					console.log('is_answered status is updated =====================');
				}, error => {
					console.log('is_answered update error =====================', error);
				})
			}, error => {
			...
			include:[
				{
					model:Models.Customers,
					as:'sales_customer_detail',
					attributes: ['id','user_id','customer_id','customer_name_en','customer_name_ar','customer_type_id']
	...
	if (req.body.user_id!=null && req.body.user_id!="" && req.body.employee_id!=null && req.body.employee_id!="") {
		Models.SurveyQuestionAnswers.findAll({
			where:{
  			...
  			attributes: ['survey_id', Sequelize.fn('count', Sequelize.col('survey_id'))],
			group: ['survey_id'],
		}).then(surveysCount => {
	   			...
	   			Models.SalesOrderRequest.count({
					...
					Models.SalesOrderRequest.count({
						...
						res.end(JSON.stringify({
			                response: 1,
			                message: Messages['en'].SUCCESS_FETCH,
			                salesData: { pendingCount:pendingCount, acceptedCount:acceptedCount, rejectedCount:rejectedCount, totalPayment:totalPayment }
			            }));
					},error =>{
						res.end(JSON.stringify({
				            response: 0,
	   			...
	   			Models.SalesOrderRequest.count({
					...
					Models.SalesOrderRequest.count({
						...
						res.end(JSON.stringify({
			                response: 1,
			                message: Messages['en'].SUCCESS_FETCH,
			                invoiceData: { pendingCount:pendingCount, acceptedCount:acceptedCount, rejectedCount:rejectedCount, totalPayment:totalPayment }
			            }));
					},error =>{
						res.end(JSON.stringify({
				            response: 0,
	   			...
	   			Models.SalesOrderRequest.count({
					...
					Models.SalesOrderRequest.count({
						...
						res.end(JSON.stringify({
			                response: 1,
			                message: Messages['en'].SUCCESS_FETCH,
			                returnInvoiceData: { pendingCount:pendingCount, acceptedCount:acceptedCount, rejectedCount:rejectedCount, totalPayment:totalPayment }
			            }));
					},error =>{
						res.end(JSON.stringify({
				            response: 0,
...
exports.net_payment_report = function(req, res){
	if (req.body.user_id!=null && req.body.user_id!="" && req.body.employee_id!=null && req.body.employee_id!="") {
		...
		Models.CustomerPayments.findAll({
			...
			attributes: [[Sequelize.fn('SUM', Sequelize.col('amount')), 'total']],
		}).then(totalCashPayment => {
            Models.CustomerPayments.count({
				where:{
					user_id:req.body.user_id,
					employee_id:req.body.employee_id,
				},
			}).then(cashPaymentCount => {
				// console.log("payment data ===========================",totalPayment);
				// res.end(JSON.stringify({
	   //              response: 1,
	   //              message: Messages['en'].SUCCESS_FETCH,
	   ...
	   //          }));
	   			Models.CustomerPayments.findAll({
					...
					}).then(chequePaymentCount => {
						res.end(JSON.stringify({
			                response: 1,
			                message: Messages['en'].SUCCESS_FETCH,
			                cashPaymentdata: { cashPaymentCount:cashPaymentCount, totalCashPayment:totalCashPayment },
			                chequePaymentdata: { chequePaymentCount:chequePaymentCount, totalChequePayment:totalChequePayment }
			            }));
					},error =>{
						res.end(JSON.stringify({
				            response: 0,
...
exports.net_sales_report = function(req, res){
	if (req.body.user_id!=null && req.body.user_id!="" && req.body.employee_id!=null && req.body.employee_id!="") {
		...
		Models.SalesOrderRequest.findAll({
			...
			attributes: [[Sequelize.fn('SUM', Sequelize.col('total_price')), 'total_price']],
		}).then(totalInvoice => {
            Models.SalesOrderRequest.count({
				where:{
					user_id:req.body.user_id,
	   			...
	   			Models.SalesOrderRequest.findAll({
					...
					}).then(returnInvoiceCount => {
						res.end(JSON.stringify({
			                response: 1,
			                message: Messages['en'].SUCCESS_FETCH,
			                invoicedata: { invoiceCount:invoiceCount, totalInvoice:totalInvoice },
			                returnInvoicedata: { returnInvoiceCount:returnInvoiceCount, totalReturnInvoice:totalReturnInvoice }
			            }));
					},error =>{
						console.log("error =========================",error)
						res.end(JSON.stringify({
	...
	if (req.body.request_id!=null && req.body.request_id!="") {
		Models.SalesOrderRequest.findOne({
			...
			include:[{
				model:Models.Customers,
				as:'sales_customer_detail',
				attributes: ['id','user_id','app_employee_id','customer_id','customer_name_en','customer_name_ar']
				
			},{
				model:Models.Users,
				as:'branch_detail',
				attributes: ['id','branch_name_en','branch_name_ar','branch_location','email','phone_no','vat_no','fax_no']
			},{
				model:Models.Employees,
				as:'employee_detail_in_sales_order_request',
				...
				include:[{
					model:Models.Items,
					as:'item_detail_in_sales_request_detail',
					attributes: ['id','user_id','item_name_en','item_name_ar','item_number','selling_price']
				},{
					model:Models.SalesOrderRequestDetailPromotion,
					as:'promotions_in_sales_order_request_details'
				}]
...
exports.get_category_data = function(req, res){
	if (req.body.employee_id!=null && req.body.employee_id!="" && req.body.category_id!=null && req.body.category_id!="") {
		Models.SalesOrderRequestDetail.findAll({
			where:{
				category_id:req.body.category_id,
				employee_id:req.body.employee_id,
			},
			attributes: [[Sequelize.fn('SUM', Sequelize.col('total_price_with_tax')), 'total']],
			// group: ['item_id'],
		}).then(categoryData => {
			res.end(JSON.stringify({
                response: 1,
	...
	if (req.body.payment_id!=null && req.body.payment_id!="") {
		Models.CustomerPayments.findOne({
			...
			include:[{
				...
				model:Models.Customers,
				as:'customer_detail_in_customer_payments',
				attributes: ['id','user_id','app_employee_id','customer_id','customer_name_en','customer_name_ar','credit_limit']
...
exports.get_route_customers = function(req, res){
	console.log("req body data ==================== ",req.body)
	if (req.body.user_id!=null && req.body.user_id!="" && req.body.route_id!=null && req.body.route_id!="") {
	...
	if (req.body.user_id!=null && req.body.user_id!="" && req.body.employee_id!=null && req.body.employee_id!="" && req.body.customer_id!=null && req.body.customer_id!="" && req.body.no_of_days!=null && req.body.no_of_days!="") {
		Models.Timelines.findAll({
			where:{
				...
				customer_id:req.body.customer_id,
				content_type: {
			      [Op.or]: ['customer_payments', 'return_invoice_order', 'invoice_order']
			    },
			 	created_at: {
			      $gte: moment().subtract(req.body.no_of_days, 'days').toDate()
			    }
			...
			Models.Customers.findOne({
				...
				attributes: ['id', 'customer_id', 'customer_name_en', 'customer_name_ar']
			}).then(customerData => {
				Models.Users.findOne({
					where:{
						id:req.body.user_id
					...
					include:[{
						...
						as:'company_detail_in_branch',
						attributes: ['id','company_name_en','company_name_ar','created_at']
					}],
				}).then(userData => {
					Models.CustomerPayments.findAll({
						where:{
			            ...
			            Models.SalesOrderRequest.findAll({
				            ...
				            Models.SalesOrderRequest.findAll({
					            ...
					            console.log("total return invoice =============== ",totalReturnInvoice)
					            res.end(JSON.stringify({
				                        response: 1,
				                        message: Messages['en'].SUCCESS_FETCH,
				                        result:{totalPayment:totalPayment,totalInvoice:totalInvoice,totalReturnInvoice:totalReturnInvoice,userData:userData,customerData:customerData,statementData:statementData}
				                    }));
							},error =>{
								res.end(JSON.stringify({
						            response: 0,
...
exports.send_account_statement_to_customer = async function(req, res){
	...
	console.log("acc statement req body file ================== ",req.files.file)

	if (req.body.user_id!=null && req.body.user_id!="" && req.body.employee_id!=null && req.body.employee_id!="" && req.body.customer_id!=null && req.body.customer_id!="" && req.body.email!=null && req.body.email!="" && req.files) {

		let image = req.files.file;
		filename1 = Date.now() + '_' + Math.floor(10000 + Math.random() * 90000)+"."+file_type_regex.exec(image.name)[1] ;
        req.body.filename = filename1;

        await _uploadImage(image, LOCAL_IMAGES_URL+'account_statements/' + filename1, filename1, function(resp_type, response, extra){
            if(resp_type == "success"){
                console.log(extra)
                // req.body.image_name = filename1;
            }
